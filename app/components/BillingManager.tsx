...
export default function BillingManager({ clientId, clientName }: BillingManagerProps) {
  ...
  const handleAddBilling = async () => {
    if (!newBilling.amount || !newBilling.billing_date) {
      alert('Please fill in required fields (Amount and Billing Date)');
      return;
    }
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-8">
          ...
          <p className="mt-1 text-sm text-gray-500">Get started by creating a new billing record.</p>
        </div>
      ) : (
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-600">
                    <div>Billed: {formatDate(record.billing_date)}</div>
                    {record.due_date && (
                      <div>Due: {formatDate(record.due_date)}</div>
                    )}
                    {record.description && (
